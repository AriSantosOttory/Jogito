...
class MainMenu(Menu):
    ...
    def display_menu(self):  # Aparência do Menu
        ...
        while self.run_display:
            ...
            self.game.draw_text('-JOGITO-', 40, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 3)
            self.game.draw_text('PROJETO DE CONCLUSÃO DE CURSO DE PHYTON', 18, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 15)
            self.game.draw_text("Rui Monteiro", 12, self.mid_w - 200, self.mid_h -160)
            self.game.draw_text("Manuel Marinho", 12, self.mid_w + 200, self.mid_h  -160)
            self.game.draw_text("Formador", 10, self.mid_w - 200, self.mid_h -140)
            self.game.draw_text("Formador", 10, self.mid_w + 200, self.mid_h  -140)
...
class OptionsMenu(Menu):
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.game.draw_text("Disparar Bullet:  F", 15, self.shotx, self.shoty)
            self.game.draw_text("Pular: ↑ ", 15, self.shotz, self.shoth)
            self.game.draw_text("Voltar: Backspace", 10, self.mid_w - 200, self.mid_h + 190)
            self.game.draw_text("Avançar: Enter", 10, self.mid_w + 200, self.mid_h + 190)
            self.blit_screen()
...
class CreditsMenu(Menu):
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.game.draw_text('DESENVOLVIDO POR:', 20, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 20 )
            self.game.draw_text('ARI SANTOS', 18, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 + -150)
            self.game.draw_text('Game Test:', 12, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 + -60)
            self.game.draw_text('- Eliel Fernandes -', 13, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 + -26)
            self.game.draw_text('MENTORIA E SUPERVISÃO DE:', 20, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 + 30)
            self.game.draw_text('DIOGO OLIVEIRA', 18, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 + 85)
            self.game.draw_text('Game Develope - Unity,Unreal', 12, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 + 120)
            self.game.draw_text('Mindera', 12, self.game.DISPLAY_W / 2, self.game.DISPLAY_H / 2 + 135)
            self.game.draw_text("Voltar: Backspace", 10, self.mid_w - 200, self.mid_h + 190)
            self.game.draw_text("Avançar: Enter", 10, self.mid_w + 200, self.mid_h + 190)
            self.blit_screen()